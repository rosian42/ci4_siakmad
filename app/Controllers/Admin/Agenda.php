...
class Agenda extends BaseController
{
    function __construct()
    {
        ...
        $this->halaman_controller = "agenda";
        $this->halaman_label = "Agenda";
    }
            ...
            if(empty($this->request->getVar('post_id'))){
                $aturan = [
                    ...
                    'event_date' => [
                        'rules' => 'required',
                        'errors' => [
                            'required'=>'Tanggal dilaksanakannya agenda harus diisi'
                        ]
                    ],
                    'post_status' => [
                        'rules' => 'required',
                ...
                $dataAgenda = $this->m_post->getPost($this->request->getVar('post_id'));// ambil data
                $aturan = [
                    'post_title' => [
                        'rules' => 'required',
                        'errors' => [
                ...
                if(!$this->validate($aturan)){
                    ...
                    $post_thumbnail = $dataAgenda['post_thumbnail'];
                    if($file->getName()){
                        $nm_file = $file->getRandomName();
                        $path = 'upload/'.$this->halaman_controller;
                        $post_thumbnail = $path.'/'.$nm_file;
                        $file->move($path, $nm_file);
                        @unlink($dataAgenda['post_thumbnail']);
                    }
                    $record = [
                        'post_id' => $dataAgenda['post_id'],
                        'username' => session()->get('akun_username'),
                        'post_title' => $this->request->getVar('post_title'),
                        'event_date' => $this->request->getVar('event_date'),
                        'post_status' => $this->request->getVar('post_status'),
